module mod_pa_boys
   ...
   ! TODO: check if 15 is sufficient for required Schwarz integrals
   integer(i4), parameter :: BOYS_N_MAX = 15
   ...
   integer(i4), parameter :: table_factor = 1/table_step
   ! Beyond this x-value the boys_factorial is used.
   real(dp), parameter :: table_x_asym = 27.0d0
   integer(i4), parameter :: table_add_points = 10
   ! Number of x-points in the table
   integer(i4), parameter :: table_points = table_x_asym/table_step + table_add_points
...
contains
   ...
   pure real(dp) function boys_neville(N, x)
      ...
      real(dp), intent(in) :: x
      real(dp) :: xs(5), ys(5)
      integer(i4) :: x_closest
      integer(i4) :: indices(5)
      real(dp) :: x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11

      x_closest = 1 + int(x*table_factor, i4)
